...
PGM/YAML 仍为 Nav2 用的单通道占用栅格；本工具**不修改**它们。可生成：

- **`*_gray_person_composite.png`**（`--composite-only`，巡航存图默认）：浅灰底 + 洋红实心人点，整体较亮。
- **`*_person_overlay.png`**（默认 CLI）：半透明圆 + 黄十字；底图默认轻度提亮（`--overlay-no-brighten` 可关）。

用法示例::

  ros2 run human_yolo_seg annotate_saved_map_person_overlay \\
    data/maps/semantic_pre_strip/coverage_map_20260109.yaml ~/.ros/tj_person_strip_regions.yaml
"""
...
from human_yolo_seg.tools.strip_saved_map import _load_map_meta, _world_to_uv
...
def _brighten_occ_gray_for_display(
    gray: np.ndarray, *, gain: float = 1.12, bias: float = 24.0
) -> np.ndarray:
    """占栅 PGM 常偏暗；仅用于可视化 PNG，不改变磁盘上的 PGM。"""
    g = gray.astype(np.float32) * float(gain) + float(bias)
...
def build_person_overlay_bgr(
    ...
    *,
    ...
    draw_outline: bool,
    cross_size: int,
    brighten_base: bool = True,
) -> np.ndarray:
    """gray: HxW uint8 occupancy; returns HxWx3 BGR."""
    g0 = _brighten_occ_gray_for_display(gray) if brighten_base else gray
    base = cv2.cvtColor(g0, cv2.COLOR_GRAY2BGR)
    h = gray.shape[0]
...
def main() -> None:
    ...
    ap.add_argument(
        "--composite-only",
        action="store_true",
        help="只输出 <stem>_gray_person_composite.png（浅灰底+实心人点），不写 person_overlay",
    )
    ap.add_argument(
        "--also-composite",
        action="store_true",
        help="在写出 person_overlay 的同时另存 gray_person_composite（与 --composite-only 互斥）",
    )
    ap.add_argument(
    ...
    ap.add_argument(
        "--overlay-no-brighten",
        action="store_true",
        help="person_overlay 底图不做提亮（恢复偏暗、与旧版一致）",
    )
    args = ap.parse_args()
    ...
    def _write_composite_png(dest: Path) -> None:
        ...
        if not cv2.imwrite(str(dest), comp_bgr):
            raise OSError(f"cv2.imwrite 失败: {dest}")
        print(f"写入 {dest}（gray_person_composite，共 {len(regions)} 个区域）")

    if args.composite_only:
        if args.output is not None:
            comp_path = args.output.resolve()
        else:
            comp_path = map_yaml.parent / f"{map_yaml.stem}_gray_person_composite.png"
        _write_composite_png(comp_path)
        return
    ...
    bgr = build_person_overlay_bgr(
        ...
        brighten_base=not bool(args.overlay_no_brighten),
    )
    ...
    print(
        f"写入 {out_path}（共 {len(regions)} 个区域；若圆与墙错位可试 --no-flip-y；"
        f"底图偏亮为默认提亮，可 --overlay-no-brighten）"
    )

    if args.also_composite:
        comp = map_yaml.parent / f"{map_yaml.stem}_gray_person_composite.png"
        _write_composite_png(comp)
