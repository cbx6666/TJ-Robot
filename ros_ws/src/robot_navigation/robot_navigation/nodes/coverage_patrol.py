...
class CoveragePatrol(Node):
    def __init__(self) -> None:
        ...
        # 存图成功后调用 strip_saved_map_person_free，生成 *_person_free.pgm/.yaml（需人物区域 YAML）
        self.declare_parameter('post_save_strip_person_free', 'false')
        self.declare_parameter('strip_saved_map_no_flip_y', 'false')
        ...
        self._post_save_strip_person_free = _as_bool(
            self.get_parameter('post_save_strip_person_free').value
        )
        self._strip_saved_map_no_flip_y = _as_bool(
            self.get_parameter('strip_saved_map_no_flip_y').value
        )
        ...
        self.get_logger().info(
            ...
            f'post_strip_person_free={int(self._post_save_strip_person_free)}'
        )
        if self._save_map_on_complete:
            try:
                ...
                self.get_logger().info(
                    ...
                    '推荐在脚本里设置到 data/maps/baseline_raw 或 data/maps/semantic_pre_strip）: '
                    f'{sd}'
                )
            except OSError as e:
                self.get_logger().warning(f'coverage_patrol: 无法创建存图目录: {e}')
        if self._post_save_person_overlay or self._post_save_strip_person_free:
            self.get_logger().info(
    ...
    def _resolve_map_save_directory(self) -> Path:
        return resolve_map_save_directory(self._save_map_directory_raw)
    ...
    def _ensure_person_regions_yaml(self, map_yaml: Path) -> Optional[Path]:
        """用于 overlay / strip：优先 ~/.ros/tj_person_strip_regions.yaml，否则按需从点云快照。"""
        regions = Path(self._person_regions_yaml)
        if regions.is_file():
            return regions
        if not self._snapshot_overlay_from_cloud:
            return None
        snap_yaml = map_yaml.parent / f'{map_yaml.stem}_person_regions_snapshot.yaml'
        self.get_logger().info(
            f'person_regions: 无 recorder 文件 {regions}，'
    ...
    def _maybe_post_save_person_overlay(
        self, map_yaml: Path, regions: Optional[Path]
    ) -> None:
        ...
        if regions is None:
            self.get_logger().info(
                ...
                '且云快照未生成或已关闭 snapshot；请起 person_strip_recorder，'
                '或确保仿真中曾发布 person_laser_map_cloud）'
            )
            return
        cmd = [
            'ros2',
        ...
        self.get_logger().info(f'person map PNG (gray_person_composite): {" ".join(cmd)}')
        try:
            r = subprocess.run(
                cmd,
                capture_output=True,
    ...
    def _maybe_post_save_strip_person_free(
        self, map_yaml: Path, regions: Optional[Path]
    ) -> None:
        if not self._post_save_strip_person_free:
            return
        if regions is None:
            self.get_logger().warning(
                'post_save_strip_person_free: skip（无人物区域 YAML；'
                '与 overlay 相同：需 ~/.ros/tj_person_strip_regions.yaml 或 '
                'snapshot_overlay_from_cloud_if_no_regions=true 且点云话题曾有人向数据）'
            )
            return
        cmd = [
            'ros2',
        ...
        if self._strip_saved_map_no_flip_y:
            cmd.append('--no-flip-y')
        self.get_logger().info(f'strip_saved_map_person_free: {" ".join(cmd)}')
        try:
            r = subprocess.run(
                cmd,
                capture_output=True,
            ...
            self.get_logger().warning(f'strip_saved_map_person_free failed to run: {e}')
            return
        if r.returncode != 0:
            err = (r.stderr or r.stdout or '').strip()
            self.get_logger().warning(f'strip_saved_map_person_free exit {r.returncode}: {err}')
            return
        tail = (r.stdout or '').strip()
        if tail:
            self.get_logger().info(tail)
    ...
    def _run_map_saver(self) -> None:
        ...
        base = resolve_map_basename(self._save_map_basename_raw)
        ...
        map_yaml_path = Path(full_stub).with_suffix('.yaml')
        regions_pp: Optional[Path] = None
        if self._post_save_person_overlay or self._post_save_strip_person_free:
            regions_pp = self._ensure_person_regions_yaml(map_yaml_path)
        self._maybe_post_save_person_overlay(map_yaml_path, regions_pp)
        self._maybe_post_save_strip_person_free(map_yaml_path, regions_pp)
        self._shutdown_if_exit_after_save()
