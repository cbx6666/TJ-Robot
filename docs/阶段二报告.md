...
* **成功** ：机器人按顺序到达全部（或≥80%）预置航点，且未因不可达目标导致提前终止。
* **失败** ：巡航中途被 stop 指令取消、Nav2 任务持续失败导致放弃，或超时未完成。
...
- **成功**：成功检测到目标物体，生成有效地图坐标点，任务状态流转至 `EXECUTING` 并下发抓取指令。
- **失败**：YOLO 未检测到目标、目标点投影到不可通行区域（墙上）、LLM 解析失败或任务超时。
